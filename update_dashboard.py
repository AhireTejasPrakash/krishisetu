...
# I will replace the component logic with a completely new structure that includes custom tasks and location search.
...
export default function Dashboard() {
  const { t, userName, userCrop, setUserCrop, setIsLoggedIn } = useApp()
  ...
  const fetchLocation = () => {
    ...
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          try {
            const res = await fetch(`http://127.0.0.1:8000/weather?lat=${position.coords.latitude}&lon=${position.coords.longitude}`)
  ...
  const fetchWeatherByCity = async (city: string) => {
    setIsLoadingWeather(true)
    try {
      const res = await fetch(`http://127.0.0.1:8000/weather/by-city?city=${encodeURIComponent(city)}`)
      const weatherData = await res.json()
  ...
  const generateAdvice = (temp: number) => {
    let ad = []
    if (temp > 35) {
      ad.push({ title: "High Heat Alert", desc: "Temperatures are very high. Ensure frequent irrigation and avoid spraying chemicals in the afternoon." })
    } else {
      ad.push({ title: "Weather is Optimal", desc: "Good conditions for general farming activities." })
    }
    
    if (userCrop) {
      ad.push({ title: `Crop Care: ${userCrop}`, desc: `Monitor ${userCrop} leaves for early signs of pests.` })
    }
    return ad
  }
  ...
  const fetchMandiPrice = async (crop: string) => {
    try {
      const res = await fetch(`http://127.0.0.1:8000/api/mandi-price?crop=${crop}`)
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const res = await fetch("http://127.0.0.1:8000/api/analyze-plant", { method: "POST", body: formData })
      if (res.ok) {
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex justify-between items-center mb-4">
        <div>
          ...
          <p className="text-charcoal-500">{t('dashboard.greetingSubtitle')}</p>
        </div>
        <button
